 ...
 */
class syntax_plugin_isowikitweaks_list extends DokuWiki_Syntax_Plugin {
	...
	public function connectTo($mode) {
		$this->Lexer->addEntryPattern('<list.*?>(?=.*?</list>)',$mode,'plugin_isowikitweaks_list');
	}
	public function postConnect() {
		$this->Lexer->addExitPattern('</list>','plugin_isowikitweaks_list');
	}
	...
	public function handle($match, $state, $pos, Doku_Handler $handler){
		switch ($state) {
			case DOKU_LEXER_ENTER:
				$attrs = $this->element_attributes('list', $match);
